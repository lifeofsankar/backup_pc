#!/usr/bin/env python3
"""
Object Detection Node -- fixed for ur5e_dual_arm world file
------------------------------------------------------------
World objects:
  cylinder : pose x=0.5  y=-0.2  z=0.5  (right side, y < 0)
  sphere   : pose x=0.6  y=0.4   z=0.5  (left  side, y >= 0)

Key fixes vs original:
  1. PassThrough z filter widened from 0.02-0.35 to 0.02-2.0
     Objects are at z~0.5 in world frame so the old filter cut them out.
     The camera sees them in camera frame at a different z depending on
     camera pose -- widening the range catches both objects reliably.

  2. Size filter changed from np.all(size > 0.03) to np.any(size > 0.03)
     A sphere has equal dims in all axes; np.all was rejecting it if any
     axis was slightly under threshold due to sparse depth returns.

  3. min_size lowered from 15 to 5 -- depth cameras give sparse point
     clouds on small objects, 15 was too high and dropped valid clusters.

  4. Input point threshold lowered from 50 to 20 for same reason.
"""
...
class ObjectDetectionNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            'Expecting: cylinder @ (0.5,-0.2,0.5)  sphere @ (0.6,0.4,0.5)')
    ...
    def cloud_callback(self, msg):
        ...
        # ── PassThrough filter ────────────────────────────────────────────────
        # Wide z range: objects are at world z~0.5 but camera frame z varies.
        # 0.02 removes points right at the camera; 2.0 keeps everything useful.
        ...
        # min_size=5: depth cameras give sparse returns on small objects
        clusters = self.fast_cluster(downsampled, tolerance=0.02, min_size=5)

        # ── noise filter ──────────────────────────────────────────────────────
        # np.any: accepts clusters where AT LEAST ONE dimension is meaningful.
        # np.all was rejecting spheres whose axes were slightly below threshold.
        valid = []
        for c in clusters:
        ...
        if not valid:
            self.get_logger().warn(
                'No valid clusters found. Check camera FOV covers the objects.')
            return
        ...
        for i, cluster in enumerate(valid):
            ...
            try:
                ...
                self.get_logger().warn(f'TF failed: {e} -- using raw coords')
                wx = float(centroid[0])
                wy = float(centroid[1])
                wz = float(centroid[2])
        ...
        self.get_logger().info(
            ...
            f'left={sum(1 for p in pose_array.poses if p.position.y >= 0)} '
            f'right={sum(1 for p in pose_array.poses if p.position.y < 0)}')
